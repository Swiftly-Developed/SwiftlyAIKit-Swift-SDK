...
struct AIErrorTests {
    ...
    func testAuthenticationErrors() {
        for error in SampleErrors.authenticationErrors {
            ...
            #expect(error.localizedDescription.count > 0, "Should have description")
        }
    }
    ...
    func testAllErrorsHaveDescriptions() {
        for error in SampleErrors.allErrors {
            #expect(error.localizedDescription.count > 0, "Error should have description: \(error)")
        }
    }
    ...
    func testAllErrorsEquatable() {
        for error in SampleErrors.allErrors {
            #expect(error == error, "Error should equal itself")
        }
    }
}
