...
/// End-to-end tests for `GrokProvider` streaming: SSE reassembly, cumulative content,
/// streamed tool-call reconstruction, stop-reason mapping, and — critically — surfacing
/// the terminal `{"choices":[],"usage":{…}}` chunk under real xAI (OpenAI-compatible
/// `include_usage`) framing.
@Suite("GrokProvider Streaming Tests")
struct GrokStreamingTests {
    ...
    func testStreamAssemblesContentAndSurfacesTerminalUsage() async throws {
        let responses = try await collect(MockGrokAPI.streamingResponseRealFraming)
        ...
        // This assertion is RED before the streaming hoist and GREEN after.
    ...
    func testStreamReassemblesToolCalls() async throws {
        let responses = try await collect(MockGrokAPI.streamingToolCallResponseRealFraming)
    ...
    func testAccumulateToolCallsStaticReassembly() {
        var accumulated: [GrokToolCall] = []

        GrokProvider.accumulateToolCalls([
            GrokDeltaToolCall(
                index: 0,
                id: "call_1",
                type: "function",
                function: GrokDeltaFunctionCall(name: "get_weather", arguments: "{\"loc")
            )
        ], into: &accumulated)
        ...
        GrokProvider.accumulateToolCalls([
            GrokDeltaToolCall(
                index: 0,
                function: GrokDeltaFunctionCall(name: nil, arguments: "ation\":\"SF\"}")
            )
        ], into: &accumulated)
    ...
    func testAccumulateMultipleToolCalls() {
        ...
        GrokProvider.accumulateToolCalls([
            GrokDeltaToolCall(index: 0, id: "call_a", type: "function",
                              function: GrokDeltaFunctionCall(name: "first", arguments: "{}")),
            GrokDeltaToolCall(index: 1, id: "call_b", type: "function",
                              function: GrokDeltaFunctionCall(name: "second", arguments: "{}"))
        ], into: &accumulated)
